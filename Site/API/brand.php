...
if (strlen($_SESSION['login']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$file_name = Uploadimage('brand', 'image');
		$field['b_title'] = addslashes($_POST['b_title']);
		$field['b_detail'] = addslashes($_POST['b_detail']);
		$field['b_logo'] = $file_name;

		$res = Insertdata("brand", $field);
		if ($res != 0) {
			$msg = "Success ";
		} else {
			$error = "Deleted ";
	...
	if (isset($_POST['update'])) {
		...
		$res = Updatedata("brand", $field, "b_id =$eid");
		if ($res != 0) {
	...
	if ($_GET['action'] == 'del' && $_GET['scid']) {
		$id = intval($_GET['scid']);
		$res = Deletedata("delete from brand where b_id='$id'");
	...
	if ($_GET['action'] == 'status' && $_GET['scid']) {
		...
		$val = $_GET['val'];
		$field['b_active'] = $val;
		$res = Updatedata("brand", $field, "b_id=$scid");
		$msg = "Success ";
		//	header("location: ".$_SERVER['PHP_SELF']);
	}
						...
						<!-- container START-->
										...
										<div class="card-body">
											<table data-order='[[ 0, "desc" ]]' id="basic-datatable"
												...
												<tbody>
													...
													$listdata = Selectdata("Select * FROM brand");
													...
													if (sizeof($listdata) == 0) {
														...
														foreach ($listdata as $row) {
															...
															<tr>
																...
																<td>
																	<?php image($row['b_logo'], '90', '70', 'cover'); ?>

																</td>

																<td>
																	<?php text($row['b_title'], $strong = true, $small = false, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>

																<td>
																	<?php text(substr($row['b_detail'], 0 , 25), $strong = false, $small = true, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>

																<td>
																	<?php active($row['b_active'], $row['b_id']); ?>
																</td>
							...
							<?php if ($edit == 1) { ?>
								...
								<div class="col-lg-12">
									...
									<?php
									$listdata = Selectdata("Select * FROM brand WHERE b_id=$eid");
									foreach ($listdata as $row) {
										?>
												...
												<div class="card">
													<div class="card-body">
														<div class="row">
															...
															<?php forminput('Title', 'b_title', $row['b_title'], 'required', '12', 'text'); ?>
															<?php formtextarea('Detail', 'b_detail', $row['b_detail'], 'required', '12', 'text'); ?>
							...
							<?php forminput('Name', 'b_title', '', 'required', '6', 'text'); ?>

							<?php formtextarea('Detail', 'b_detail', '', 'required', '12', 'text'); ?>
