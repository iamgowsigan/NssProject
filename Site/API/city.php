...
if (strlen($_SESSION['login']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$field['district_id'] = $districtid;
		$field['city_title'] = addslashes($_POST['city_title']);


		$res = Insertdata("city", $field);
		if ($res != 0) {
			$msg = "Success ";
		} else {
			$error = "Deleted ";
	...
	if (isset($_POST['update'])) {
		...
		$res = Updatedata("city", $field, "city_id =$eid");
		if ($res != 0) {
	...
	if ($_GET['action'] == 'del' && $_GET['scid']) {
		$id = intval($_GET['scid']);
		$res = Deletedata("delete from city where city_id='$id'");
						...
						<!-- container START-->
										...
										<div class="card-body">
											<table data-order='[[ 0, "desc" ]]' id="basic-datatable"
												...
												<tbody>
													...
													$listdata = Selectdata("Select * FROM city WHERE district_id = '$districtid' AND state_id = '$stateid'");
													...
													if (sizeof($listdata) == 0) {
														...
														foreach ($listdata as $row) {
															...
															<tr>
																...
																<td>
																	<?php text($row['city_id'], $strong = false, $small = false, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>

																<td>
																	<?php text($row['city_title'], $strong = true, $small = false, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>

																<td>
																	<?php action($row['city_id'], 'E,D'); ?>
																</td>
							...
							<?php if ($edit == 1) { ?>
								...
								<div class="col-lg-12">
									...
									<?php
									$listdata = Selectdata("Select * FROM city WHERE city_id=$eid");
									foreach ($listdata as $row) {
										?>
												...
												<div class="card">
													<div class="card-body">
														<div class="row">
															...
															<?php forminput('City', 'city_title', $row['city_title'], 'required', '12', 'text'); ?>
							...
							<?php forminput('City', 'city_title', '', 'required', '6', 'text'); ?>
