...
class _MapScreenState extends State<MapScreen> with RelativeScale {
  ...
  Marker locMarker=Marker(markerId: MarkerId("Here"),);
  ...
  void initState() {
    ...
    _kInitialPosition = CameraPosition(target:_kMapCenter, zoom: 18.0, tilt: 0, bearing: 0);
    getLocation();
  ...
  getLocation() async {
    ...
    setState(() {
      _kMapCenter = LatLng(_locationData!.latitude!.toDouble(), _locationData!.longitude!.toDouble());
      _kInitialPosition = CameraPosition(target: _kMapCenter, zoom: 18.0, tilt: 0, bearing: 0);
    });
    ...
    return Container(
        ...
        child: Stack(
          children: [
                    ...
                    Expanded(child:  Stack(
                      children: [
                        ...
                        Align(
                          ...
                          child: Icon(Icons.gps_fixed,size: sy(25),color: ThemePrimary,),
                        )
                      ],
                    ),),
            ...
            Positioned(
              ...
              child:  Column(
                children: [
                            ...
                            child:Row(
                              ...
                              children: [
                                GestureDetector(
                                  ...
                                  child: Icon(Icons.arrow_back,size: sy(l),color: fc_3,),
                                ),
                                SizedBox(width: sy(7),),
                                ...
                                (ETsearch.text!='') ? GestureDetector(
                                  ...
                                  child: Icon(Icons.gps_fixed,size: sy(l),color: fc_4,),
                                )
                              ],
                            )
                  ...
                  if(_placeList.length!=0)Container(
                    ...
                    child: Column(
                      children: [
                        for(int i=0;i<_placeList.length;i++)
                         GestureDetector(
                           onTap: (){
                             getPlaceName(_placeList[i]["place_id"].toString());
                               ...
                               children: [
                                Row(
                                  children: [
                                    ...
                                    Icon(Icons.map_outlined,size: sy(xl),color: Colors.grey.shade400,),
                                    SizedBox(width: sy(5),),
                                    Expanded(child: Text(_placeList[i]["description"],style: ts_Regular(sy(n),fc_3),),
                                    )
                                  ],
                                ),
                                 if(i!=_placeList.length-1)Container(
                                   margin: EdgeInsets.fromLTRB(sy(0), sy(5), sy(0), sy(0)),
                                   width: Width(context),height: sy(1),color: Colors.grey.shade300,)
                               ],
                             )
                           ),
                         )
            ...
            if(_keyboardVisible == false) Positioned(
                bottom: 0,
                left: 0,
                right: 0,
                child: addressBar(),)
  ...
  //  placemarks = await placemarkFromCoordinates(locMarker.position.latitude, locMarker.position.longitude);
    placemarks = await placemarkFromCoordinates(_kMapCenter.latitude, _kMapCenter.longitude);
                    ...
                    child: Column(
                      ...
                      children: [
                        Expanded(child: SingleChildScrollView(
                          child:  Text(address,style: ts_Regular(sy(n), fc_1),),
                          ...
                          style: elevatedButton(ThemePrimary, sy(5)),
                            onPressed: (){
                              setState(() {
                                ...
                                try{
                                  address=ETname.text.toString()+','+ETstreet.text.toString();
                                  address=address.toString().replaceAll("'", ' ');
                                  address=address.toString().replaceAll('"', ' ');
                                  address=address.toString().replaceAll('/', ' ');
                                  Navigator.of(context).pop(
                                      {'lat':(_kMapCenter.latitude.toString()=='')?'24.319831':_kMapCenter.latitude.toString(),
                                        'long':(_kMapCenter.longitude.toString()=='')?'54.799529':_kMapCenter.longitude.toString(),
  ...
  getPlaceName(String input) async {
    ...
    String url = 'https://maps.googleapis.com/maps/api/place/details/json?place_id=$input&key=$kPLACES_API_KEY';
    ...
    setState(() {
      _kMapCenter = LatLng(double.parse(lat.toString()),double.parse(lng.toString()));
      _mapcontroller.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: _kMapCenter,zoom: 13)));
      FocusScope.of(context).unfocus();
      _placeList.clear();
      ETsearch.clear();
